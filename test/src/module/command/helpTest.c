...
testRun(void)
{
    ...
    if (testBegin("helpRender()"))
    {
        ...
        const char *commandHelp = zNewFmt(
            ...
            "  --repo-gcs-sts-host                 GCS security token service host\n"
            "  --repo-gcs-user-project             GCS project ID\n"
            "  --repo-gcs-web-id-audience          GCS Workload Identity Federation provider\n"
            "                                      resource name\n"
            "  --repo-gcs-web-id-token-file        GCS web identity JWT file\n"
